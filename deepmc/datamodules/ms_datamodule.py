...
class TSDataModule(LightningDataModule):
    """LightningDataModule for MS_Synthetic dataset.
...
class MODataModule(LightningDataModule):
    """LightningDataModule for MS_Synthetic dataset.
...
class oursDataModule(LightningDataModule):
    """LightningDataModule for MS_Synthetic dataset.
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        # load and split datasets only if not loaded already
        if not self.data_train and not self.data_val and not self.data_test:
            self.data_train = ours_Dataset(
                self.hparams.data_dir,
                train=True,
                topology=self.hparams.topology,
                mode=self.hparams.mode,
                lazy=self.hparams.lazy,
                subset=self.hparams.subset
            )
            self.data_val = ours_Dataset(
                self.hparams.data_dir,
                train=False,
                topology=self.hparams.topology,
                mode=self.hparams.mode,
