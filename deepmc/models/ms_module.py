...
from torchmetrics import MinMetric, MeanAbsoluteError, MeanMetric
import numpy as np

from deepmc.utils.metrics import TranslationalError
from deepmc.utils.MoCap_Solver.utils import vertex_loss, angle_loss, HuberLoss, Criterion_EE
...
class MOLitModule(LightningModule):
    ...
    def __init__(
        ...
        # use separate metric instance for train, val and test step
        # to ensure a proper reduction over the epoch
        self.train_pos_err = MeanMetric()
        self.train_ang_err = MeanMetric()
        self.val_pos_err = MeanMetric()
        self.val_ang_err = MeanMetric()
        self.val_err = MeanMetric()
        self.test_pos_err = MeanMetric()
        self.test_ang_err = MeanMetric()
    ...
    def step(self, batch: Any):
        ...
        reconst_loss1 = self.hparams.beta_3 * self.criterion_rec(Y_m_norm, X_m_norm)
        reconst_loss2 = self.hparams.beta_4 * self.criterion_rec(Y_m[:, -3:, :], X_m[:, -3:, :])
        reconst_loss3 = self.hparams.beta_4 * self.criterion_rec(J_t_glob, J_t_glob_gt)

        loss = (reconst_loss1 + (reconst_loss2 * self.hparams.beta_1 + reconst_loss3 * self.hparams.beta_2) * self.hparams.beta_3) * self.hparams.beta_4
    ...
    def training_step(self, batch: Any, batch_idx: int):
        ...
        # log train metrics
        pos_error = self.criterion_ver(J_t_glob, J_t_glob_gt)
        angle_error = self.criterion_ang(J_R_glob, J_R_glob_gt)
        self.train_pos_err.update(pos_error)
        self.train_ang_err.update(angle_error)
        self.log("train/loss", loss, on_step=False, on_epoch=True, prog_bar=False)
        self.log("train/pos_err", pos_error, on_step=False, on_epoch=True, prog_bar=True)
        self.log("train/ang_err", angle_error, on_step=False, on_epoch=True, prog_bar=True)
    ...
    def validation_step(self, batch: Any, batch_idx: int):
        ...
        pos_error = self.criterion_ver(J_t_glob, J_t_glob_gt)
        angle_error = self.criterion_ang(J_R_glob, J_R_glob_gt)
        self.val_err.update(loss)
        self.val_pos_err.update(pos_error)
        self.val_ang_err.update(angle_error)
        self.log("val/loss", loss, on_step=False, on_epoch=True, prog_bar=False)
        self.log("val/pos_err", pos_error, on_step=False, on_epoch=True, prog_bar=True)
        self.log("val/ang_err", angle_error, on_step=False, on_epoch=True, prog_bar=True)
    ...
    def test_step(self, batch: Any, batch_idx: int):
        ...
        pos_error = self.criterion_ver(J_t_glob, J_t_glob_gt)
        angle_error = self.criterion_ang(J_R_glob, J_R_glob_gt)
        self.test_pos_err.update(pos_error)
        self.test_ang_err.update(angle_error)
        self.log("test/loss", loss, on_step=False, on_epoch=True)
...
class MSLitModule(LightningModule):
    ...
    def __init__(
        ...
        clean_markers_stat_dir = data_dir + "/MS_Synthetic_preprocessed/clean_markers_statistics.npy"
        first_rot_stat_dir = data_dir + "/MS_Synthetic_preprocessed/first_rot_statistics.npy"
        ts_stat_dir = data_dir + "/MS_Synthetic_preprocessed/ts_statistics.npy"
        mc_stat_dir = data_dir + "/MS_Synthetic_preprocessed/mc_statistics.npy"
        mo_stat_dir = data_dir + "/MS_Synthetic_preprocessed/mo_statistics.npy"
        ts_latent_stat_dir = data_dir + "/MS_Synthetic_preprocessed/ts_latent_statistics.npy"
        mc_latent_stat_dir = data_dir + "/MS_Synthetic_preprocessed/mc_latent_statistics.npy"
        mo_latent_stat_dir = data_dir + "/MS_Synthetic_preprocessed/mo_latent_statistics.npy"
        skinning_weights_dir = data_dir + "/MS_Synthetic_preprocessed/skinning_weights.npy"

        clean_markers_mean, clean_markers_std = torch.tensor(np.load(clean_markers_stat_dir), dtype=torch.float32)
        first_rot_mean, first_rot_std = torch.tensor(np.load(first_rot_stat_dir), dtype=torch.float32)
        ts_data_mean, ts_data_std = torch.tensor(np.load(ts_stat_dir), dtype=torch.float32)
        mc_data_mean, mc_data_std = torch.tensor(np.load(mc_stat_dir), dtype=torch.float32)
        mo_data_mean, mo_data_std = torch.tensor(np.load(mo_stat_dir), dtype=torch.float32)[:, None, :, None]
        ts_latent_mean, ts_latent_std = torch.tensor(np.load(ts_latent_stat_dir), dtype=torch.float32)
        mc_latent_mean, mc_latent_std = torch.tensor(np.load(mc_latent_stat_dir), dtype=torch.float32)
        mo_latent_mean, mo_latent_std = torch.tensor(np.load(mo_latent_stat_dir), dtype=torch.float32)
        skinning_weights = torch.tensor(np.load(skinning_weights_dir), dtype=torch.float32)

        lambda_para = torch.tensor(self.encoder_net.lambda_para[None, None, ...], dtype=torch.float32)
        lambda_para_jt = torch.tensor(self.encoder_net.lambda_jt_para[None, None, ...], dtype=torch.float32)
        lambda_para_jt1 = torch.tensor(self.encoder_net.lambda_jt_para[None, None, ..., None], dtype=torch.float32)
        ...
        # use separate metric instance for train, val and test step
        # to ensure a proper reduction over the epoch
        self.train_jpe = MeanMetric()
        self.train_joe = MeanMetric()
        self.train_mpe = MeanMetric()
        self.val_jpe = MeanMetric()
        self.val_joe = MeanMetric()
        self.val_mpe = MeanMetric()
        self.val_err = MeanMetric()
        self.test_jpe = MeanMetric()
        self.test_joe = MeanMetric()
        self.test_mpe = MeanMetric()
    ...
    def step(self, batch: Any):
        raw_marker, clean_marker, skeleton_pos, motion, offsets, marker_config, first_rot, of_code, mc_code, transform = batch
        ...
        out_markerconf = self.mc_encoder_net.decoder_net(l_c, res_offset_output_input, l_t)
        ...
        res_pos, res_rot = self.fk.forward_from_raw2(res_motion, out_offset[0], out_first_rot, world=True, quater=True)
        res_markers = self.skin.skinning(res_markerconf, self.skinning_weights[None, ...], res_rot, res_pos)
        ...
        mrk_pos_loss = self.criterion_huber(clean_marker * self.lambda_para, res_markers * self.lambda_para)
        skel_pos_loss = self.criterion_huber(skeleton_pos * self.lambda_para_jt, res_pos * self.lambda_para_jt)
        first_rot_xform_loss = self.criterion_huber(first_rot_xform, res_first_rot_xform)
        motion_xform_loss = self.criterion_huber(transform * self.lambda_para_jt1, res_rot * self.lambda_para_jt1)
        offset_latent_loss = self.criterion_huber(of_code, l_t)
        motion_loss = self.criterion_huber(res_motion[..., :-3], motion[..., :-3])
        motion_trans_loss = self.criterion_huber(res_motion[:, -3:, :].permute(0, 2, 1), motion[:, -3:, :].permute(0, 2, 1))
        marker_latent_loss = self.criterion_huber(mc_code, l_c)
        marker_config_loss = self.criterion_huber(marker_config[:, :, :, :3], res_markerconf)
        offset_loss = self.criterion_huber(offsets * self.lambda_para_jt, out_offset[0] * self.lambda_para_jt)

        loss = (mrk_pos_loss * self.hparams.beta_1
        ...
        return loss, clean_marker, res_markers, transform, res_rot, skeleton_pos, res_pos

    def training_step(self, batch: Any, batch_idx: int):
        loss, clean_markers, res_markers, transform, res_rot, skeleton_pos, res_pos = self.step(batch)

        # log train metrics
        joint_pos_error = self.criterion_ver(skeleton_pos, res_pos)
        joint_ori_error = self.criterion_ang(transform, res_rot)
        marker_pos_error = self.criterion_ver(clean_markers, res_markers)
        self.train_jpe.update(joint_pos_error)
        self.train_joe.update(joint_ori_error)
        self.train_mpe.update(marker_pos_error)
        self.log("train/loss", loss, on_step=False, on_epoch=True, prog_bar=False)
    ...
    def validation_step(self, batch: Any, batch_idx: int):
        ...
        marker_pos_error = self.criterion_ver(clean_markers, res_markers)
        self.val_err.update(loss)
        self.val_jpe.update(joint_pos_error)
        self.val_joe.update(joint_ori_error)
        self.val_mpe.update(marker_pos_error)
        self.log("val/loss", loss, on_step=False, on_epoch=True, prog_bar=False)
        self.log("val/jpe", joint_pos_error, on_step=False, on_epoch=True, prog_bar=True)
        self.log("val/joe", joint_ori_error, on_step=False, on_epoch=True, prog_bar=True)
        self.log("val/mpe", marker_pos_error, on_step=False, on_epoch=True, prog_bar=True)
    ...
    def test_step(self, batch: Any, batch_idx: int):
        ...
        joint_ori_error = self.criterion_ang(transform, res_rot)
        marker_pos_error = self.criterion_ver(clean_markers, res_markers)
        self.test_jpe.update(joint_pos_error)
        self.test_joe.update(joint_ori_error)
        self.test_mpe.update(marker_pos_error)
        self.log("test/loss", loss, on_step=False, on_epoch=True)
